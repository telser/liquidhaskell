...
lemma2 n x
  ...
  = trivial 
  | otherwise
  = lemma2 (n-1) (ack n (x-1)) &&& lemma2 n (x-1)
...
lemma4 x n
  =  lemma2 (n+1) (x-1) &&& lemma3_gen n x (ack (n+1) (x-1))
...
lemma6 h n x
  ...
  = trivial
  | h > 0
  = lemma6 (h-1) n x &&& lemma3_gen n (iack (h-1) n x) (iack (h-1) n (x+1))
...
lemma9 n x l
  ...
  = lemma9_helper x l 
  | otherwise
  = lemma4_gen 1 n x &&& lemma9_helper x l
...
lemma10 n x l
  | n == 0
  = lemma10_zero l x &&& lemma10_one x
  | l == 0
  = lemma2 (n+1) x
  | otherwise
  =   def_eq n x
...
ladder_prop1 n l x
    ...
    = trivial
    | otherwise
    = ladder_prop1 n (l-1) x &&& def_eq (n-1) (ladder (l-1) n x)
...
ladder_prop3 x y n l
  =  ladder_prop1 n l x &&& ladder_prop1 n l y &&& lemma6_gen l n x y
